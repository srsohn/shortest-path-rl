...
class Const(object):
  ...
  assert OBSERVATION_HEIGHT%4 == 0
  assert OBSERVATION_WIDTH%4 == 0
  ...
  LEVELS = [
      ...
      Level('contributed/dmlab30/explore_goal_locations_small',
            fully_qualified_name='explore_goal_locations_small',
            random_maze=True,
            include_in_paper=True,
            scenarios=['goal_small']),

      # Two levels where we expect to show episodic curiosity does not hurt.
      # Corresponds to 'Dense 1' setting in the paper.
      Level('contributed/dmlab30/explore_object_rewards_many',
            fully_qualified_name='explore_object_rewards_many',
            include_in_paper=True,
            scenarios=['dense_explore']),
      Level('contributed/dmlab30/explore_object_rewards_few',
            fully_qualified_name='explore_object_rewards_few',
            include_in_paper=True,
            scenarios=['sparse_explore']),
  ...
  @staticmethod
  def get_max_episode_len_by_envname(env_name):
    if env_name.startswith('dmlab'):
      return 1350
    else:
      return None

  @staticmethod
  def is_episode_identical(environment_engine, env_name):
