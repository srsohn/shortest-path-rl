...
import numpy as np
import third_party.baselines.common.tf_util as U
from third_party.baselines.a2c.utils import fc
...
class CategoricalPdType(PdType):
    ...
    def pdfromlatent(self, latent_vector, init_scale=1.0, init_bias=0.0):
        ...
        # self.pdfromflat (flat)
        # -> self.pdclass()(flat) == CategoricalPd(logits=flat) "Why? See above pdclass(self) func"
...
class MaskedCategoricalPdType(CategoricalPdType):
    def pdfromlatent(self, latent_vector, mask=None, init_scale=1.0, init_bias=0.0):
        pdparam = fc(latent_vector, 'pi', self.ncat, init_scale=init_scale, init_bias=init_bias)
        if mask is not None:  # masking
            masked_pdparam = pdparam + tf.log(mask)  # mask = [0, 1] --> log(mask) = [-infty, 0]
            # This pdparam is logit of softmax
        else:
            masked_pdparam = pdparam
        return self.pdfromflat(masked_pdparam), masked_pdparam, self.pdfromflat(pdparam), pdparam
...
def make_pdtype(ac_space, masking=False):
    ...
    if isinstance(ac_space, spaces.Box):
        ...
        if masking:
            return MaskedCategoricalPdType(ac_space.n)
        else:
