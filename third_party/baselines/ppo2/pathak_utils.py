...
@gin.configurable
def universeHead(x, nConvs=4, trainable=True, resize=True):
  ...
  if len(x.shape) == 2:
    for i in range(nConvs):
      x = tf.nn.relu(linear(x, int(x.shape[1]), 'l{}'.format(i+1),
...
def icm_forward_model(encoded_state, action, num_actions, hidden_layer_size, one_hot=True):
  if one_hot:
...
def icm_forward_output(encoded_state, hidden):
  pred_next_state = linear(hidden, encoded_state.get_shape()[1].value, 'flast',
                          normalized_columns_initializer(0.01))
  return pred_next_state


def icm_inverse_model(encoded_state, encoded_next_state,
                      hidden_layer_size):
...
def icm_inverse_output(hidden, num_actions):
